 ...
 * @see also modbusrtu_t
 */
enum MBrtu_FC {
  MBrtu_FC_NONE                     = 0,   /*!< null operator */
  MBrtu_FC_READ_COILS               = 1,	/*!< FCT=1 -> read coils or digital outputs */
  MBrtu_FC_READ_DISCRETE_INPUT      = 2,	/*!< FCT=2 -> read digital inputs */
  MBrtu_FC_READ_REGISTERS           = 3,	/*!< FCT=3 -> read registers or analog outputs */
  MBrtu_FC_READ_INPUT_REGISTER      = 4,	/*!< FCT=4 -> read analog inputs */
  MBrtu_FC_WRITE_COIL               = 5,	/*!< FCT=5 -> write single coil or output */
  MBrtu_FC_WRITE_REGISTER           = 6,	/*!< FCT=6 -> write single register */
  MBrtu_FC_WRITE_MULTIPLE_COILS     = 15,	/*!< FCT=15 -> write multiple coils or outputs */
  MBrtu_FC_WRITE_MULTIPLE_REGISTERS = 16	/*!< FCT=16 -> write multiple registers */
};
...
const unsigned char fctsupported[] = { 
  MBrtu_FC_READ_COILS,
  MBrtu_FC_READ_DISCRETE_INPUT,
  MBrtu_FC_READ_REGISTERS, 
  MBrtu_FC_READ_INPUT_REGISTER,
  MBrtu_FC_WRITE_COIL,
  MBrtu_FC_WRITE_REGISTER, 
  MBrtu_FC_WRITE_MULTIPLE_COILS,
  MBrtu_FC_WRITE_MULTIPLE_REGISTERS
};
...
public:
  ModbusRtuV2(); 
  ModbusRtuV2(uint8_t u8id, uint8_t u8serno); 
  ModbusRtuV2(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin);
  ...
  int8_t query( modbusrtu_t telegram ); //!<only for master
 ...
 */
ModbusRtuV2::ModbusRtuV2(uint8_t u8id, uint8_t u8serno) {
  init(u8id, u8serno, 0);
}
 ...
 * @overload Modbus::Modbus()
 */
ModbusRtuV2::ModbusRtuV2(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin) {
 ...
 * @ingroup setup
 */
void ModbusRtuV2::begin(long u32speed) {
 ...
 * @ingroup setup
 */
void ModbusRtuV2::begin(long u32speed,uint8_t u8config) {
 ...
 * @ingroup setup
 */
void ModbusRtuV2::setID( uint8_t u8id) {
  if (( u8id != 0) && (u8id <= 247)) {
    this->u8id = u8id;
  }
}
 ...
 * @ingroup setup
 */
void ModbusRtuV2::setTimeOut( uint16_t u16timeOut) {
 ...
 * @ingroup loop
 */
boolean ModbusRtuV2::getTimeOutState() {
 ...
 * @ingroup buffer
 */
uint16_t ModbusRtuV2::getOutCnt() { 
  return u16OutCnt; 
}
 ...
 * @ingroup buffer
 */
uint16_t ModbusRtuV2::getErrCnt() { 
  return u16errCnt; 
}
 ...
 * @ingroup buffer
 */
uint8_t ModbusRtuV2::getLastError() {
  return u8lastError;
}
 ...
 * @brief
 * *** Only Modbus Master ***
 * Generate a query to an slave with a modbusrtu_t telegram structure
 * The Master must be in COM_IDLE mode. After it, its state would be COM_WAITING.
 * This method has to be called only in loop() section.
 * 
 * @see modbusrtu_t 
 * @param modbusrtu_t  modbus telegram structure (id, fct, ...)
 * @ingroup loop
 * @todo finish function 15
 */
int8_t ModbusRtuV2::query( modbusrtu_t telegram ) {
  ...
  case MBrtu_FC_READ_COILS:
  case MBrtu_FC_READ_DISCRETE_INPUT:
  case MBrtu_FC_READ_REGISTERS:
  case MBrtu_FC_READ_INPUT_REGISTER:
    au8Buffer[ NB_HI ]      = highByte(telegram.u16CoilsNo );
    au8Buffer[ NB_LO ]      = lowByte( telegram.u16CoilsNo );
    u8BufferSize = 6;
    break;
  case MBrtu_FC_WRITE_COIL:
    au8Buffer[ NB_HI ]      = ((au16regs[0] > 0) ? 0xff : 0);
    au8Buffer[ NB_LO ]      = 0;
    u8BufferSize = 6;    
    break;
  case MBrtu_FC_WRITE_REGISTER:
    au8Buffer[ NB_HI ]      = highByte(au16regs[0]);
    au8Buffer[ NB_LO ]      = lowByte(au16regs[0]);
    u8BufferSize = 6;    
    break;
  case MBrtu_FC_WRITE_MULTIPLE_COILS: // TODO: implement "sending coils"
  ...
  case MBrtu_FC_WRITE_MULTIPLE_REGISTERS:
    au8Buffer[ NB_HI ]      = highByte(telegram.u16CoilsNo );
    au8Buffer[ NB_LO ]      = lowByte( telegram.u16CoilsNo );
 ...
 * as defined in its modbusrtu_t query telegram.
 * 
 * @params	nothing
 * @return errors counter
 * @ingroup loop
 */
int8_t ModbusRtuV2::poll() {
  ...
  case MBrtu_FC_WRITE_COIL:
  case MBrtu_FC_WRITE_REGISTER :
  case MBrtu_FC_WRITE_MULTIPLE_COILS:
  case MBrtu_FC_WRITE_MULTIPLE_REGISTERS :
    // nothing to do
    break;
  default:
 ...
 * @ingroup loop
 */
int8_t ModbusRtuV2::poll( uint16_t *regs, uint8_t u8size ) {
...
void ModbusRtuV2::init(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin) {
  this->u8id = u8id;
 ...
 * @ingroup buffer
 */
int8_t ModbusRtuV2::getRxBuffer() {
 ...
 * @ingroup buffer
 */
uint16_t ModbusRtuV2::calcCRC(uint8_t u8length) {
 ...
 * @ingroup buffer
 */
uint8_t ModbusRtuV2::validateRequest() {
 ...
 * @ingroup buffer
 */
uint8_t ModbusRtuV2::validateAnswer() {
  // check message crc vs calculated crc
  uint16_t u16MsgCRC = 
    ((au8Buffer[u8BufferSize - 2] << 8) 
    | au8Buffer[u8BufferSize - 1]); // combine the crc Low & High bytes
 ...
 * @ingroup buffer
 */
void ModbusRtuV2::buildException( uint8_t u8exception ) {
 ...
 * @ingroup discrete
 */
int8_t ModbusRtuV2::process_FC1( uint16_t *regs, uint8_t u8size ) {
 ...
 * @ingroup register
 */
int8_t ModbusRtuV2::process_FC3( uint16_t *regs, uint8_t u8size ) {
 ...
 * @ingroup discrete
 */
int8_t ModbusRtuV2::process_FC5( uint16_t *regs, uint8_t u8size ) {
 ...
 * @ingroup register
 */
int8_t ModbusRtuV2::process_FC6( uint16_t *regs, uint8_t u8size ) {
 ...
 * @ingroup discrete
 */
int8_t ModbusRtuV2::process_FC15( uint16_t *regs, uint8_t u8size ) {
 ...
 * @ingroup register
 */
int8_t ModbusRtuV2::process_FC16( uint16_t *regs, uint8_t u8size ) {
  uint8_t u8func = au8Buffer[ FUNC ];  // get the original FUNC code
